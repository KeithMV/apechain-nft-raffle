...
/**
 * @title RaffleFactoryBaseV3 - Direct Contract Creation for Base Network
 * @dev Factory that creates contracts directly (no cloning) for Base compatibility
 */
contract RaffleFactoryBaseV3 is Ownable, ReentrancyGuard, Pausable {
    
    uint256 public platformFee = 500; // 5% (matching V4)
    ...
    uint256 public constant RATE_LIMIT = 10; // 10 seconds (matching V4)
    ...
    function createRaffle(
        ...
        // CREATE CONTRACT DIRECTLY (NO CLONING)
        RaffleContractSecureV3 raffleContract = new RaffleContractSecureV3();
        
        // Transfer NFT to raffle contract
        nft.transferFrom(msg.sender, address(raffleContract), tokenId);
        
        // Initialize raffle
        raffleContract.initialize(
            nftContract,
            tokenId,
            msg.sender,
            ticketPrice,
        ...
        // Store mappings
        raffleContracts[raffleCounter] = address(raffleContract);
        validRaffles[address(raffleContract)] = true;
    ...
    // Emergency withdrawal for accidentally sent funds
