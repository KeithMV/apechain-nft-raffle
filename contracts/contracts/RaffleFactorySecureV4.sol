...
/**
 * @title RaffleFactorySecureV4 - Reduced Rate Limiting
 * @dev Factory contract with 10-second rate limit for faster raffle creation
 */
contract RaffleFactorySecureV4 is Ownable, ReentrancyGuard, Pausable {
    
    uint256 public platformFee = 500; // 5% (updated default)
    ...
    // Rate limiting - REDUCED FROM 300 TO 10 SECONDS
    ...
    function createRaffle(
        ...
        // Rate limiting - now only 10 seconds
        require(
    ...
    receive() external payable {
        // Only accept payments from valid raffles
        require(validRaffles[msg.sender], "Unauthorized");
    }
}