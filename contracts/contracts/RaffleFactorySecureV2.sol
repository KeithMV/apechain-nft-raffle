...
import "./RaffleContractSecureV2.sol";

/**
 * @title RaffleFactorySecureV2 - Enhanced Security NFT Raffle Factory
 * @dev Factory contract with comprehensive security measures
 */
contract RaffleFactorySecureV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor() {
        raffleTemplate = address(new RaffleContractSecureV2());
    }
    ...
    function createRaffle(
        ...
        // Initialize raffle
        RaffleContractSecureV2(raffleContract).initialize(
            nftContract,
            tokenId,
            msg.sender,
            ticketPrice,
    ...
    function pauseRaffle(address raffleContract) external onlyOwner {
        ...
        RaffleContractSecureV2(raffleContract).emergencyPause();
    }
    
    function unpauseRaffle(address raffleContract) external onlyOwner {
        require(validRaffles[raffleContract], "Invalid raffle");
        RaffleContractSecureV2(raffleContract).emergencyUnpause();
    }
