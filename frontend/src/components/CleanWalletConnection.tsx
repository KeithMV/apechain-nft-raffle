...
import { useApeChainConnectors, getConnectorDisplayInfo } from '../hooks/useApeChainConnectors';
...
export default function CleanWalletConnection() {
  ...
  const compatibleConnectors = useApeChainConnectors();
  ...
  if (showWallets) {
    return (
      <div className="relative">
        <div className="absolute top-0 right-0 bg-slate-800 border border-slate-700 rounded-lg p-4 shadow-xl z-50 min-w-[300px]">
          ...
          <div className="space-y-2">
            {compatibleConnectors.map((connector) => {
              const displayInfo = getConnectorDisplayInfo(connector);
              return (
                <button
                  key={connector.id}
                  onClick={() => handleConnect(connector)}
                  disabled={isPending}
                  className="w-full flex items-center space-x-3 p-3 bg-slate-700/50 hover:bg-slate-600/50 rounded-lg transition-colors disabled:opacity-50 relative"
                >
                  ...
                  <div className="text-left flex-1">
                    ...
                    <div className="text-slate-400 text-sm">{displayInfo.description}</div>
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
            ...
            <div className="text-emerald-200 text-xs">
              Only wallets that support ApeChain are shown. 
              {compatibleConnectors.length} compatible wallet{compatibleConnectors.length !== 1 ? 's' : ''} available.
            </div>
          </div>
        </div>
      </div>
